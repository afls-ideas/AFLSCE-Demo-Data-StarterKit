...
import getStatus from '@salesforce/apex/DemoReplenishmentController.getStatus';
import createReplenishment from '@salesforce/apex/DemoReplenishmentController.createReplenishment';
import deleteReplenishment from '@salesforce/apex/DemoReplenishmentController.deleteReplenishment';

export default class ReplenishmentSetup extends LightningElement {
    statusData;
    statusError;
    isLoading = false;
    resultMessage;
    ...
    async handleCreate() {
        ...
        this.activityLog = [];
        try {
            const raw = await createReplenishment();
            let result;
            try {
            ...
            this.dispatchEvent(
                new ShowToastEvent({
                    ...
                    message: this.isSuccess ? 'Inventory replenishment created.' : 'Some records failed.',
    ...
    async handleDelete() {
        ...
        this.activityLog = [];
        try {
            const result = await deleteReplenishment();
            this.isSuccess = true;
            ...
            this.dispatchEvent(
                new ShowToastEvent({
                    title: 'Success',
                    message: 'Replenishment records deleted.',
                    variant: 'success'
                })
            );
        } catch (error) {
