...
import getStatus from '@salesforce/apex/DemoInquiryController.getStatus';
import getTerritories from '@salesforce/apex/DemoInquiryController.getTerritories';
import getInquiryRecordTypes from '@salesforce/apex/DemoInquiryController.getInquiryRecordTypes';
import createInquiriesForTerritory from '@salesforce/apex/DemoInquiryController.createInquiriesForTerritory';
import deleteInquiriesForTerritory from '@salesforce/apex/DemoInquiryController.deleteInquiriesForTerritory';
import deleteAllInquiries from '@salesforce/apex/DemoInquiryController.deleteAllInquiries';

export default class InquirySetup extends LightningElement {
    statusData;
    statusError;
    isLoading = false;
    resultMessage;
    ...
    selectedResponsePreference = 'Email';
    ...
    @wire(getInquiryRecordTypes)
    wiredRecordTypes({ data, error }) {
        if (data) {
            this.recordTypeOptions = data;
            // Default to "Medical Inquiry" if present, else first option
            if (!this.selectedRecordType && data.length > 0) {
                const medical = data.find((opt) => opt.value === 'Medical Inquiry');
                this.selectedRecordType = medical ? medical.value : data[0].value;
            }
        } else if (error) {
    ...
    handleResponsePreferenceChange(event) {
        this.selectedResponsePreference = event.detail.value;
    }
    ...
    async handleCreate() {
        ...
        try {
            const raw = await createInquiriesForTerritory({
                territoryId: this.selectedTerritoryId,
                recordTypeName: this.selectedRecordType,
                responsePreference: this.selectedResponsePreference
            });
            let result;
            try {
    ...
    async handleDeleteTerritory() {
        ...
        if (!window.confirm('Delete all demo Medical Inquiries (Cases + Inquiries + Questions) for the selected territory?')) return;
        this.isLoading = true;
        this.resultMessage = undefined;
        this.activityLog = [];
        try {
            const result = await deleteInquiriesForTerritory({ territoryId: this.selectedTerritoryId });
            this.isSuccess = true;
    ...
    async handleDelete() {
        if (!window.confirm('Delete ALL demo Medical Inquiries? This cannot be undone.')) return;
        this.isLoading = true;
        this.resultMessage = undefined;
        this.activityLog = [];
        try {
            const result = await deleteAllInquiries();
            this.isSuccess = true;
            this.resultMessage = result;
            this.activityLog = [
                { id: 0, message: 'Delete Complete', isHeader: true },
